...
const addUserToGame = ({ id, name, code }) => {
  ...
  const existingUser = usersInGame.find(user => user.name.trim().toLowerCase() === name.trim().toLowerCase())
  if (existingUser) return { error: 'Username is taken' }
  if (game.started) return { error: 'The Game in in progress' }
  if (usersInGame.length === 10) return { error: 'Uh.. oh.. too many players in the game' }
